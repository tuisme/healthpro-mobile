...
class _ProductCardState extends State<HospitalCard> {
  ...
  void getFar() async {
    ...
    var url = 'https://maps.googleapis.com/maps/api/distancematrix/json?origins=' + _location.latitude.toString() + ', ' + _location.longitude.toString() + '&destinations=' + des.split(",")[0] + ',' + des.split(",")[1] + '&key=KEY';
    print(url);
    try {
      final response =
      await http.get(url);
